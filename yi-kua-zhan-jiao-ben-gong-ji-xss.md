...
内容安全策略（Content security policy, CSP）是一种浏览器策略，用于检测并防范某些特定类型的攻击，包括 XSS 和数据注入攻击等。如果应用程序启用了 CSP ，则可有效防范 XSS 的攻击利用。正常情况下，CSP 可以规避大多数潜在的 XSS 漏洞利用。
...
在一些情况下，预防 XSS 漏洞很简单。但如果根据应用程序和业务的复杂性来预防 XSS 漏洞会很艰难。

大多数情况下，预防 XSS 漏洞可以结合以下措施

* 在输入点过滤：接收用户输入的地方，尽可能严格过滤相关数据，保证相关输入的数据是符合预期要求的
* 在输出的转义：在输出点输出的响应数据是用户可控时，需要对输出数据进行编码转义后再输出，防止相关内容被解释为 JavaScript 来运行。具体输出可以根据输出环境决定，可能需要应用 HTML 、URL 、 JavaScript 和 CSS 编码的组合
* 使用响应头防护：为防止 HTTP 响应中包含可能存在的 HTML 和 JavaScript 代码，你可以使用Content-Type 和 X-Content-Type-Options 标头来确保浏览器按照你预期的方式来解释响应
* 内容安全策略（CSP）：你可以使用内容安全策略来作为最后一道防线，它可以降低可能存在的 XSS 漏洞被利用的严重性

## 1.14 一些关于 XSS 的问题

Q：XSS 漏洞常见吗？

A：XSS 漏洞非常常见，XSS 漏洞可能是最常见的 Web 安全漏洞。

Q：XSS 攻击常见吗？

A：没有被 XSS 攻击的相关可靠统计数据，但它可能比其它漏洞更难被利用。

Q：XSS 和 CSRF 的区别在哪？

A：XSS 主要是会导致网站返回恶意的 JavaScript ，而 CSRF 主要是用于诱导用户执行非他们主观的操作。

Q：XSS 和 SQL 注入的区别在哪？

A：XSS 是针对应用程序的其它用户的客户端漏洞，而 SQL 注入是针对应用程序数据库的服务器端漏洞。

Q：如何在 PHP 中预防 XSS 漏洞？

A：提示并使用白名单过滤相关的输入，或将该输入进行类型转换。在 HTML 环境中使用 htmlentities 和 ENT\_QUOTES 来进行转义输出，在 JavaScript 环境中使用 JavaScript Unicode 来进行转义输出。

Q：如何在 Java 中预防 XSS 漏洞？

A：使用白名单过滤输入，在 HTML 环境中使用可以使用 Google Guava 库来对输出进行 HTML 编码，在JavaScript 环境中使用 JavaScript Unicode 来进行转义输出。
